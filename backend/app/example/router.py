...
async def get_examples() -> List[ExampleResponse]:
    ...
    return [
        ExampleResponse(id=str(example.id), name=example.name) for example in examples
    ]
...
async def update_example(
    example_id: str, example: PutExampleRequest
) -> ExampleResponse:
    existing_example = await ExampleDocument.find_one(
        ExampleDocument.id == ObjectId(example_id)
    )
